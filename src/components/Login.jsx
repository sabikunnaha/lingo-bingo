...
const Login = () => {
    ...
    const [showpassword, setShowPassword]= useState(false)
     ...
     const from = location?.state || '/';
        ...
        const handleGoogleLogin =(singInWithGoogle)=>{
            singInWithGoogle()
           .then(result => {
                   if(result.user){
                    navigate(from, {replace:true})
        ...
        <div className='min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 flex justify-center items-center'>
            ...
            <div className="w-full max-w-md p-8  space-y-3 rounded-xl bg-gray-200  text-gray-800">
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate="" action="" className="space-y-6">
                    ...
                    <button className="block w-full p-3 text-center rounded-sm  text-gray-50  bg-blue-400">Login</button>
                </form>
                ...
                <div className="flex justify-center space-x-4">
                    <button onClick={()=>handleGoogleLogin(singInWithGoogle)} aria-label="Log in with Google" className="p-3 rounded-sm">
                ...
                <p className="text-sm text-center sm:px-6  text-gray-600">Don't have an account? Please
                    <Link to='/register' rel="noopener noreferrer" href="#" className="underline text-xl  text-blue-500">Register</Link>
                </p>
            </div>
        </div>
    );
